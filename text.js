...
S.ShapeBuilder = (function () {
  ...
  return {
    ...
    letter: function (l) {
      // Tách dòng theo ký tự \n
      var lines = String(l).split('\n');
      var s = 0;
    
      setFontSize(fontSize);
    
      // Tính cỡ chữ phù hợp theo dòng đầu tiên
      s = Math.min(
        fontSize,
        (shapeCanvas.width / shapeContext.measureText(lines[0]).width) * 0.8 * fontSize,
        (shapeCanvas.height / fontSize) * (lines.length > 1 ? 0.3 : 0.45) * fontSize
      );
      setFontSize(s);
      ...
      // Tính vị trí bắt đầu để căn giữa theo chiều dọc
      var lineHeight = s * 1.2;
      var startY = (shapeCanvas.height / 2) - (lineHeight * (lines.length - 1) / 2);
    
      // Vẽ từng dòng ra giữa canvas
      for (var i = 0; i < lines.length; i++) {
        shapeContext.fillText(lines[i], shapeCanvas.width / 2, startY + i * lineHeight);
      }
